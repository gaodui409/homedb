...
export default function HomePage() {
  ...
  // Sensors: pointer with 5px tolerance, touch with 300ms long-press
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 5 } }),
    useSensor(TouchSensor, { activationConstraint: { delay: 300, tolerance: 5 } })
  )

  // Helper: find which group a bookmark belongs to
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    // Cross-group: move bookmark instantly so the UI updates
    ...
    // Update activeDrag source group
    setActiveDrag((prev) =>
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Bookmark reorder within same group
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Quick bar: group anchors + pinned apps */}
      <QuickBar
        groups={sortedGroups}
